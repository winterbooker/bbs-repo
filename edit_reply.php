...
// 投稿者名、本文が未入力の場合はエラー表示
	...
	if( empty($_POST['reply_name']) ) {
		...
		$message_data['reply_name'] = htmlspecialchars($_POST['reply_name'], ENT_QUOTES);
	}
	...
	if( empty($_POST['reply_content']) ) {
		...
		$message_data['reply_content'] = htmlspecialchars($_POST['reply_content'], ENT_QUOTES);
	}
	...
	if( empty($error_message) ) {
		...
		if( $mysqli->connect_errno ) {
			...
			$sql = "UPDATE reply set reply_name = '$message_data[reply_name]', reply_content = '$message_data[reply_content]' WHERE reply_id = $reply_id";
			$res = $mysqli->query($sql);
		}
		...
		if( $res ) {
			header("Location: ./admin.php?page={$_GET['page_id']}#{$reply_id}");
			exit();
		}
	}
}
		...
		<div class="container">
			<form method="post">
				<div class="form_name">
					...
					<input type="text" id="reply_name" name="reply_name" placeholder="名前を入力してください" value="<?php if( !empty($message_data['reply_name']) ){ echo $message_data['reply_name']; } ?>">
				</div>
				<div class="form_content">
					<label for="reply_content">本文</label>
					<textarea id="reply_content" name="reply_content" placeholder="投稿したい内容を入力してください"><?php if( !empty($message_data['reply_content']) ){ echo $message_data['reply_content']; } ?></textarea>
				</div>
